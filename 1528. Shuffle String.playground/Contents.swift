...
// Approach with creating 2 arrays
// Time complexity - O(N)
class Solution {
    func restoreString(_ s: String, _ indices: [Int]) -> String {
        let copiedArray = Array(s)
        var resultArray = Array<Character>(repeating: "a", count: indices.count)
        for i in 0..<indices.count {
            resultArray[indices[i]] = copiedArray[i]
        }
